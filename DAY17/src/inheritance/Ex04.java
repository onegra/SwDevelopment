...
class Child4 extends Parent4 {
	...
	//기존 기능(함수)의 형식은 그대로 정의하고 내용만 새로 작성하여 사용 할 수 있다
	//기존에 정의된 함수를 새롭게 재ㅐ정이한다 라고하여 override라고 한다
	//메서드 오버라이딩은 오버로딩과 달리 반환형, 함수이름, 매개변수의 형태 및 개수가 모두 일치해야 한다
	
	//상속받은 메서드의 접근제한자 범위를 줄일수 없습니다 (늘리는 건 가능)
	// private -> package -> protected -> public
	//현재 위에 void show()가 package이므로 아래 void show()는 private를 쓸 수 없다
	
	@Override // 오버라이딩 규칙에 맞지 않으면 경고를 보내고, 실행시키지 않는다
	void show() { //이 메서드는 상속받은 후 재정의한 메서드입니다
		System.out.printf("child) db: %.2f\n",db);
	}
	                  //final 클래스는 상속 받을 수 없다 (예: scanner)
//	@Override         //final 메서드는 오버라이팅 할 수 없다 (함수 내용을 바꿀 수 없다)
//	void test () {    //final 필드는 값을 바꿀 수 있다
