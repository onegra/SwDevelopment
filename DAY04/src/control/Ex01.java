...
public class Ex01 {
	public static void main(String[] args) {
		...
		// 분기문과 반복문은 특정조건에 의해서 수행되므로 비교연산, 논리연산 등을 활용하여 boolean 타입의 값을 다루는 연습이 필요하다
		
		// >, <, >=, <=, == !=   일반적으로 수치에 대한 비교를 수행하여 boolean을 만들어낸다
		// &&, ||, !             boolean을 이용하여 새로운 boolean을 만들어낸다
		
		//사용자의 "입력값"이 무엇이냐에 따라, 어떤 크기의 값이냐에 따라 조건을 생성하고, 조건에 따라 서로 다른 코드를 수행하게 된다
		//분기문 뿐만 아니라, 반복문도 boolean 타입의 값을 사용하기 때문에 한글로 번역했을 때 "만약에"라는 형식의 구문이 등장하는 경우가 많다
		
		Scanner sc = new Scanner(System.in); //변수 선언
		int num; //입력받을 정수
		
		System.out.println("정수 입력 :"); //입력안내
		num = sc.nextInt(); //입력
		...
		while(true) { //무한반복 (조건이 항상 참이기 때문에, 계속반복한다)
