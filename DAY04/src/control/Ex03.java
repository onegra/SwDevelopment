...
public class Ex03 {
	public static void main(String[] args) {
		...
		//else는 단독으로 사용할 수 없고, 이전에 if가 나타나있어야 하고 연속되어있어야한다
		
		int age = 15;
		if(age < 20) {
			System.out.println("미성년자입니다");
		}
		//System.out.println(); //다른 코드를 작성하면 연속성이 무너지므로 else를 실핼할 수 없다
		else { //아니면(이전에 등장한 조건이 참이아니면)
		...
		if(age >= 20) {
			if(gender %2 != 0) {
				System.out.println("퀘스트에 담청되셨습니다");
			}
		}
		else {
		...
		//중첩된 제어문을 사용할 때는 들여쓰기 시작지점을 맞춰야한다
		...
		//위 코드에서 else안에 있는 if를 합친경우
		if (num == 1) {                //만약 num 의 값이 1이면
			System.out.println("one"); 
		}
